...
read -rd '' DOMAIN POSTGRESQL_REPLICAS POSTGRESQL_USERNAME POSTGRESQL_PASSWORD KAFKA_REPLICAS ZOOKEEPER_REPLICAS ELASTICSEARCH_REPLICAS < <(yq -r '.domain, .postgresql.replicas, .postgresql.username, .postgresql.password, .kafka.replicas, .zookeeper.replicas, .elasticsearch.replicas' ./cluster-config.yaml)
